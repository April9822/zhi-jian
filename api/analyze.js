...
// 🛡️ 平衡检查 Prompt
// ============================================================
const BALANCE_CHECK_PROMPT = `请检查上面的分析：1）责任占比是否基于具体行为给出了真实差异（严禁均分或50/50）；2）是否识别了所有参与者；3）每人的标签是否使用了描述而非"A/B/C"；4）是否有一方被更严厉地评判？如果是，请重新平衡；5）分析中不应出现"知间暂停解说"等元描述。如有问题请修正后返回完整JSON，只返回JSON。`;
...
export default async function handler(req, res) {
  ...
  if (!conversation || conversation.trim().length < 20) {
    return res.status(400).json({ error: '对话内容太短，请至少输入几句有意义的对话' });
  }
