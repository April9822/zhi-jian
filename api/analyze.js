...
3. 每个参与者的标签根据对话内容起名（如"先开口的一方""一直在解释的一方""沉默后爆发的一方"），不要用"A/B/C"。
4. 不劝分、不劝离、不替用户做人生决定。
5. 建议具体可操作（能直接复制使用的话术）。

## 责任占比规则
- 根据具体行为给出真实差异比例，严禁默认50/50或均分。
- 评判标准：谁先用绝对化指责？谁先翻旧账？谁先冷暴力/沉默？谁先升级情绪？
...
// 🛡️ 平衡检查 Prompt
// ============================================================
const BALANCE_CHECK_PROMPT = `请检查上面的分析：1）责任占比是否基于具体行为给出了真实差异（严禁均分或50/50）；2）是否识别了所有参与者；3）每人的标签是否使用了描述而非"A/B/C"；4）分析中不应出现"知间暂停解说"等元描述。如有问题请修正后返回完整JSON，只返回JSON。`;
