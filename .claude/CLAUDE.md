...
所有行为和产出必须遵守三级约束体系（详见 [产品核心原则](.claude/rules/product-principles.md)）：

### 三大禁止（分级，非一刀切）

1. **禁止人格诊断**（永久标签：NPD/BPD/边缘型等临床术语无条件禁止）
   → ✅ 允许描述本次对话中的沟通风格（如"在这次对话中，你倾向于回避"），需标注"仅基于本次对话"
   
2. **禁止道德审判**（判谁好谁坏："你错了""你不对"等道德判断句禁止）
   → ✅ 允许行为责任分析（如"本次冲突中你的行为贡献约40%"），需标注"仅针对本次行为"
   
3. **禁止侮辱性标签**（渣男/妈宝/扶弟魔/巨婴等贬义定性禁止）
